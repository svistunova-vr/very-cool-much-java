...
Начиная с **JDK 9** JVM также смотрит на ресурсы, но Parallel заменен на G1. При этом до JDK 27 HotSpot всё ещё мог автоматически выбрать Serial GC в ограниченной среде — например, при очень маленьком количестве CPU или памяти.
...
JVM знает только что до объекта существует путь от GC Root -> с ее точки зрения он еще нужен.
...
* **Serial GC** выполняет сборку в одном потоке и подходит прежде всего небольшим приложениям с ограниченными ресурсами.
...
Начиная с Java 9 вместо Parallel используется G1, но в ограниченных по ресурсам системах HotSpot все еще мог выбрать Serial GC.
